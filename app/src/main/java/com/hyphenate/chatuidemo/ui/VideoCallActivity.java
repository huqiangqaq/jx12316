...
//        recordBtn.setOnClickListener(this);
        ...
        List<ExperterInfo.RowsBean> infos = MyApplication.getInstance().getInfo();
        String name = null;
        if (infos!=null&&infos.size()>0){
            for (ExperterInfo.RowsBean info:infos){
                if (info.getTel().equals(username)){
                    name = info.getName();
                }
            }
        }
        nickTextView.setText(name==null?username.replaceAll("(\\d{3})\\d{4}(\\d{4})","$1****$2"):name);
