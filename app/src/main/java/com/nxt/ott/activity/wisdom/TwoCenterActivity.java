 ...
 */
public class TwoCenterActivity extends BaseTitleActivity {
    ...
    protected void initView() {
        ...
        initTopbar(this, getString(R.string.wisdom_center));

        detailstab = (TabLayout) findViewById(R.id.tab_news);
        detailsvp = (ViewPager) findViewById(R.id.vp_news);


        titles = Arrays.asList(getResources().getStringArray(R.array.two_center));
        //viewpager加载adapter
        MyFragmentAdapter adapter = new MyFragmentAdapter(getSupportFragmentManager());
        for (int i = 0; i < titles.size(); i++) {
            Log.e("Seed-47", "size-------------->" + titles.size());
            adapter.addFragment(IntroduceFragment.newInstance(i, 1), titles.get(i));//0为分类标识
        }
    ...
    protected int getLayout() {
        return R.layout.activity_two_center;
    }
